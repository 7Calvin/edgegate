...
### 2026-07-24 (v1.6.2 — Export FortiGate: single-link em SD-WAN + UX do download)
- ✅ **Single-link nasce em SD-WAN (1 membro)** — o export FortiGate do túnel sem backup já cria zona/health-check/service com 1 membro; subir pra failover depois é **aditivo** (entra o 2º membro, `members 201 202`), sem conflito de "interface in use" nem UNDO.
- ✅ **Checkbox "link de backup"** no criar/editar controla o failover e é o balizador do download; failover (2 links) marcado como **homologado só em FortiGate** (aviso p/ não usar em pfSense/Endian).
- ✅ **Download enxuto** — pede só interfaces + nome base; Local ID vem do Peer ID (`right_id`) e as redes da config; Source do SLA sempre visível.
- 🐛 Export single-link não força mais 2º link inexistente; campos do export à prova de espaço nas pontas (trim); botão **Copiar** com fallback p/ contexto HTTP (`execCommand`).
...
conn exemplo
    ...
    leftid="203.0.113.10"     # IP público (usado no PSK)
    right="198.51.100.20"    # IP do peer remoto
    rightsubnet="10.7.0.0/16" # Subnet remota
    rightid="198.51.100.20"  # ID do peer (usado no PSK)
    ike=aes256-sha256-modp2048!
    esp=aes256-sha256!
    keyexchange=ikev2
    auto=start
